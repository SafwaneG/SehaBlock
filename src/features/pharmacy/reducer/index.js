...
const reducer = (state = getInitialState(), action) => {
  ...
  return produce(state, (draft) => {
    switch (type) {
      case types.merged:
        if (
          helpers.validator.isArray(payload.pharmacyPrescriptions) &&
          payload.prescriptions.length > 0
        )
          payload.prescriptions.forEach((pharmacyPrescription, i) => {
            draft.pharmacyPrescriptions[i] = pharmacyPrescription;
          });
        break;
